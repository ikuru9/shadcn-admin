...
import { getPetByIdQueryOptions, getPetByIdSuspenseQueryOptions } from "@/gen/hooks";

const route = getRouteApi("/_authenticated/samples/pets/$id");

const PetsDetail = () => {
  const { id } = route.useParams();
  const { data: pet } = useSuspenseQuery(getPetByIdSuspenseQueryOptions({ petId: parseInt(id, 10) }));
