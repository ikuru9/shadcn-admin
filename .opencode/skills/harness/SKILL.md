...
1. `프로젝트/.opencode/agents/`, `프로젝트/.opencode/skills/`, `프로젝트/AGENTS.md`를 읽는다
...
**모델 설정:** 모든 에이전트는 `model: "openai/gpt-5.4"`를 사용한다. Agent 도구 호출 시 반드시 `model: "openai/gpt-5.4"` 파라미터를 명시한다. 하네스의 품질은 에이전트의 추론 능력에 직결되며, openai/gpt-5.4가 최고 품질을 보장한다.
...
**AGENTS.md에 넣지 않는 것:** 에이전트 목록, 스킬 목록, 디렉토리 구조, 실행 규칙 상세. 이유: 에이전트/스킬 목록은 오케스트레이터 스킬과 `.opencode/agents/`, `.opencode/skills/`에서 관리하므로 중복이다. 디렉토리 구조는 파일 시스템에서 직접 확인 가능하다. AGENTS.md는 **포인터(트리거 규칙) + 변경 이력**만 담는다.
...
- `.opencode/agents/` 파일 목록과 오케스트레이터 스킬의 에이전트 구성 비교 → 불일치 목록 생성
- `.opencode/skills/` 디렉토리 목록과 오케스트레이터 스킬의 스킬 구성 비교 → 불일치 목록 생성
- 감사 결과를 사용자에게 보고한다
...
- [ ] 모든 Agent 호출에 `model: "openai/gpt-5.4"` 파라미터 명시
